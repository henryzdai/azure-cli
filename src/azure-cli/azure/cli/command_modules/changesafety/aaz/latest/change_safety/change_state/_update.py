...
class Update(AAZCommand):
    ...
    def _handler(self, command_args):
        ...
        raw_ops = command_args.get('targets')
        super()._handler(command_args)
        extracted_targets = self._extract_targets_from_operations(raw_ops)
        if not extracted_targets:
            extracted_targets = self.ctx.args.targets.to_serializable()
        extracted_targets = [str(token) for token in extracted_targets if token is not None]
        self._raw_targets = extracted_targets
        if self._raw_targets:
            self._apply_targets_to_args(self._raw_targets)
        self._execute_operations()
        return self._output()

    def _extract_targets_from_operations(self, raw_ops):
        if raw_ops is None:
            return []
        if hasattr(raw_ops, '_ops'):
            values = []
            for keys, data in raw_ops._ops:
                if not keys:
        ...
        if isinstance(raw_ops, list):
            return [str(v) for v in raw_ops]
        return [str(raw_ops)]

    def _apply_targets_to_args(self, values):
        targets_value = self.ctx.args.targets
        targets_value.clear()
        for token in values:
            targets_value.append(token)
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        _args_schema.targets = AAZListArg(
            options=["--targets"],
            arg_group="Properties",
            help="Target definitions. Each argument can contain one or more key=value pairs separated by ',' or ';'.",
            nullable=True,
            blank=[],
        )
    ...
    def pre_operations(self):
        ...
        self.ctx.set_var('change_definition', change_definition)

    def _build_change_definition(self):
        if not self._raw_targets:
            return None
        targets = self._parse_targets(self._raw_targets)
        change_name = self.ctx.args.change_state_name.to_serializable()
        return {
            'kind': 'Targets',
            'name': change_name,
            'details': {
    ...
    def _parse_targets(raw_targets):
        values = raw_targets.to_serializable() if hasattr(raw_targets, 'to_serializable') else raw_targets
        if not values:
    ...
    def pre_instance_update(self, instance):
        props = instance.get('properties') if isinstance(instance, dict) else None
        if not isinstance(props, dict):
            return
        props.pop('targets', None)
        change_definition = getattr(self.ctx.vars, 'change_definition', None)
        if change_definition:
            props['changeDefinition'] = change_definition
    ...
    class InstanceUpdateByJson(AAZJsonInstanceUpdateOperation):
        ...
        def _update_instance(self, instance):
            ...
            if properties is not None:
                ...
                properties.set_prop("links", AAZDictType, ".links")
...
class _UpdateHelper:
    ...
    def _build_schema_change_state_read(cls, _schema):
        ...
        targets = _schema_change_state_read.properties.targets
        targets.Element = AAZStrType()
